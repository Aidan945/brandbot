...
export function createBrandbot(container: HTMLElement, options: BrandbotOptions = {}): BrandbotInstance {
  ...
  function makeCarbonTexture(size = 256, cell = 16, base = '#2c2c2c'): THREE.CanvasTexture {
    ...
    for (let y = 0; y < size / cell; y++) {
      for (let x = 0; x < size / cell; x++) {
        ...
        grad.addColorStop(0, base); grad.addColorStop(0.5, '#ffffff'); grad.addColorStop(1, base);
        g.fillStyle = grad;
  ...
  eyeCanvas.width = 512; eyeCanvas.height = 256;
    ...
    for (const cx of [102, 410]) {
      for (let row = 0; row < 5; row++) {
        for (let col = 0; col < 7; col++) {
          ...
          if (((col - 3) / 3.4) ** 2 + ((row - 2) / 2.6) ** 2 > 1) continue;
          g.beginPath(); g.arc(x, y, 6.5 * (1 - squash * 0.35), 0, Math.PI * 2); g.fill();
        }
      }
    }
  ...
  function setupModel(gltf: GLTF): void {
    ...
    if (headMesh) {
      ...
      addDecal(headMesh,
        new THREE.Vector3(hc.x, hc.y + hs.y * 0.06, hb.max.z),
        new THREE.Vector3(hs.x * 0.80, hs.x * 0.40, hs.z * 0.45),
        new THREE.MeshBasicMaterial({ map: eyeTexture, transparent: true, toneMapped: false,
    ...
    if (bodyMesh) {
      ...
      addDecal(bodyMesh,
        new THREE.Vector3(bc.x, bc.y + bs.y * 0.12, bb.max.z),
        new THREE.Vector3(bs.x * 0.55, bs.x * 0.55, bs.z * 0.5),
  ...
  const stepAngle = (cur: number, target: number, dt: number) => {
  ...
  function tick(): void {
    ...
    if (spinState.active) {
      ...
      if (k >= 1) { spinState.active = false; robot.rotation.y = 0; }
    } else {
  ...
  function set(o: BrandbotUpdate = {}): BrandbotInstance {
    ...
    if (o.accent) materials.joint.color.set(o.accent);
    if (o.visor)  materials.visor.color.set(o.visor);
    if (o.hands)  materials.chrome.color.set(o.hands);
    if (o.eyes)   drawEyes(o.eyes);
    if (o.logoColor) { state.logoColor = o.logoColor; drawLogoText(state.logoText, state.logoColor); }
    if ('logoText' in o && o.logoText !== undefined) { state.logoText = o.logoText; drawLogoText(o.logoText, state.logoColor); }
    if (o.logoImage) {
      const img = new Image();
